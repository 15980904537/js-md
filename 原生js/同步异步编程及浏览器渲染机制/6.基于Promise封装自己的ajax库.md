1.  步骤：
    ...
    - 构建出整体架构
      ...
      - ajax({config})/ajax.all([]) /ajax.get(url,config)/ajax.post(url,data,config)

        - ajax.all 函数传进去的是一个数组或者类数组，他直接返回 Promise.all 即可

        - ajax.get 最后返回的让它执行 ajax({config})

        - ajax.post 最后返回的让它执行 ajax({config})
    ...
    - 参数处理
      ...
      ```
          ...
          + 对config设定一些规则：必传required，类型type，默认项defaults

          + 按照配置项规则进行校验和合并

            + 我们没有传递值，如果要求是必须传递的，则报错，否则使用默认值即可

            + 如果传递值，则根据类型判断是否符合，不符合报错，符合根据自己传递的值处理

    - 内部核心功能的实现

          + 在原型上的方法request上实现ajax的四步法

              + xhr=new XMLHttpRequest

              + xhr.open(method,url,false)

                  + 如果是get请求，需要在url后面添加参数

                      + 判断params参数，保证参数存在且不是一个空对象，但是是对象
              ...
              + xhr.onreadystatechange=()=>{}
                  ...
                  + 这里可以拿到响应主体和响应头的信息，但是在此之前需要做一些判断

                     + 如果是请求失败

                         + 网络层失败，就会走onerror事件【网络断开】和ontimeout事件【请求超时】

                         + 如果是状态码失败，则可以拿到状态码，但是还是认为失败
                  ...
                  + 处理响应信息
                       ...
                       - headers=====》响应头信息【要把响应头的信息拼接成对象的形式返回】
...
(function () {
  ...
  class Ajax {
    ...
    request() {
      ...
      //4.2 处理响应拦截器
      let [onResolveCallback, onRejectCallback] =
    ...
    //3.2 处理url
    handleURL() {
      let { baseUrl, url, method, cache, params } = this.config;
      url = baseUrl + url;
      //params
  ...
  const initParams = function initParams(config) {
    ...
    _.each(configRule, (index, item) => {
      let { type, defaults, required } = item;
    ...
    if (!methods.includes(config["method"].toLowerCase()))
      throw new TypeError(
        "method must be a get/head/options/delete/post/put/patch!"
      );
    return config;
  };
  ...
  ajax.all = function (promises) {
    ...
    if (!_.isArrayLike(promises))
      throw new TypeError("promises must be a array or like-array!");
